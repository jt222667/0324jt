...
function [LP, SV, flag] = check_kinematics_0325(LP, SV, Goal)
%% 初始化
% 目标点容差设置
norm_limit = 1e-2;
...
parfor k = 1:num_trials
    ...
    w_ref = w_init_struct(find(Goal.change==1)) + 1e-9; 

    % 调用 fmincon
    [q_opt, fval] = fmincon(@(q) joint_IK_cost_0325(q, LP, SV, Goal, w_ref), ...
        q_init, [], [], [], [], ...
